...
{else}
    ...
    <h3 class="contact-form">{l s='Send a message'}</h3>
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		<fieldset>
            ...
            <p class="text">
				...
				{else}
					<input type="text" id="email" name="from" value="{$email}" /><strong style=" color:red">&nbsp;&nbsp;*</strong>
				{/if}
			</p>
			<p class="select">
		...
		{if !$PS_CATALOG_MODE}
			...
			<p class="text">
				<label for="id_order">{l s='Order NO.'}</label>
		...
		<p class="textarea">
			<label for="message">{l s='Message:'}</label>
			 <textarea id="message" name="message" rows="15" cols="20" style="width:370px;height:150px">{if isset($message)}{$message|escape:'htmlall':'UTF-8'|stripslashes}{/if}</textarea><strong style=" color:red">&nbsp;&nbsp;*</strong>
		</p>
        <p class="champ_requis">
        Fields marked with an asterisk (
        <strong style="color:red">*</strong>
        ) are required&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="submitMessage" id="submitMessage" value="{l s='Send'}" class="button_large" onclick="$(this).hide();" />
        </p>

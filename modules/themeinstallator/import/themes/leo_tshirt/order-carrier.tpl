...
<form id="form" action="{$link->getPageLink('order.php', true)}" method="post" onsubmit="return acceptCGV();" style="padding: 10px;">
{else}
		...
		{if isset($carriers)}
			{foreach from=$carriers item=carrier name=myLoop}
				<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
					...
					<td class="carrier_name" data-label="{l s='Carrier'}">
					...
					<td class="carrier_infos" data-label="{l s='Information'}">{$carrier.delay|escape:'htmlall':'UTF-8'}</td>
					<td class="carrier_price" data-label="{l s='Price'}">
						{if $carrier.price}
							<span class="price">

...
class prestaloveeasymenu extends Module
{
  ...
  public function installDb()
  {
    ...
    Db::getInstance()->ExecuteS("
    INSERT INTO `"._DB_PREFIX_."pl_menu` (`id_pl_menu`, `name`, `link`, `parent`, `target`, `publish`, `order`) VALUES
		(1, '', 'index.php', 0, 'none', 1, 0),
		(2, '', 'prices-drop.php', 0, 'none', 1, 0),
		(3, '', 'new-products.php', 0, 'none', 1, 3),
		(4, '', 'stores.php', 0, 'none', 1, 4),
		(5, '', 'cms.php?id_cms=4', 0, 'none', 1, 5),
		(6, '', 'contact-form.php', 0, 'none', 1, 6);
	");
   Db::getInstance()->ExecuteS('
    ...
    Db::getInstance()->ExecuteS("
		INSERT INTO `"._DB_PREFIX_."pl_menu_lang` (`id_pl_menu`, `id_lang`, `name`) VALUES
		(1, 1, 'Home'),
		(1, 2, ''),
		(1, 3, ''),
		(1, 4, ''),
	...
	public function getContent()
	{
		...
		if(Tools::isSubmit('submitBlockPLMenuEdited'))
    	{
   		...
   		<fieldset>
      		<legend><img src="../img/admin/add.gif" alt="" title="" />'.$this->l('Add/Edit ST Menu Link').'</legend>
      			<form action="'.$_SERVER['REQUEST_URI'].'" method="post" id="form">
					...
					foreach ($languages as $language)
					{
						...
						if(isset($pl_menu[0]['id_pl_menu']) && $pl_menu[0]['id_pl_menu']) {
  			...
  			<div class="margin-form">
          <input type="text" name="link" value="'.(isset($pl_menu[0]['link']) ? $pl_menu[0]['link'] : '').'" size="70" />
  			</div>
			<label>'.$this->l('Parent').'</label>
  			<div class="margin-form">
				...
				$this->_html .= $this->displaySelectboxMenu((isset($pl_menu[0]['parent']) ? $pl_menu[0]['parent'] : ''));
          ...
          <select name="target">
		    <option>none</option>
		    <option value="_blank"';
			if(isset($pl_menu[0]['target']) && $pl_menu[0]['target'] == '_blank') {
				$this->_html .= ' SELECTED';
			}
			$this->_html .= '>_blank</option>
		    <option value="_parent"';
			if(isset($pl_menu[0]['target']) && $pl_menu[0]['target']=='_parent') {
				$this->_html .= ' SELECTED';
			}
			$this->_html .= '>_parent</option>
		    <option value="_self"';
			if(isset($pl_menu[0]['target']) && $pl_menu[0]['target']=='_self') {
				$this->_html .= ' SELECTED';
			}
			$this->_html .= '>_self</option>
		    <option value="_top"';
			if(isset($pl_menu[0]['target']) && $pl_menu[0]['target']=='_top') {
				$this->_html .= ' SELECTED ';
			}
			$this->_html .= '>_top</option>
  			...
  			<div class="margin-form">';
			if(isset($pl_menu[0]['publish']) && isset($pl_menu[0]['id_pl_menu']) && ($pl_menu[0]['publish'] or !$pl_menu[0]['id_pl_menu'])) {
			...
			<label>'.$this->l('Order').'</label>
  			<div class="margin-form">
          <input type="text" name="order" value="'.(isset($pl_menu[0]['order']) ? $pl_menu[0]['order'] : '').'" size="10" />
  			</div>
        <p class="center"> ';
		if(!isset($pl_menu[0]['id_pl_menu']) || !$pl_menu[0]['id_pl_menu']) {
            $this->_html .= '
		    ...
		    $this->_html .= '<input type="hidden" name="id_pl_menu_edit" value="'.(isset($pl_menu[0]['id_pl_menu']) ? $pl_menu[0]['id_pl_menu'] : '').'" /><input type="hidden" name="parent_old" value="'.(isset($pl_menu[0]['parent']) ? $pl_menu[0]['parent'] : '').'" /><input type="submit" name="submitBlockPLMenuEdited" value="'.$this->l('  Edit  ').'" class="button" /> ';
		}
	$this->_html .= '
        </p>
  		</form>
  ...
  private function remove($id_pl_menu)
  {
	...
	private function displayLofListMenu($parent = 0, $class = 'lof-easy-menu')
	{
    	global $cookie;
		...
		$curent_link = str_replace(__PS_BASE_URI__, '', $_SERVER["REQUEST_URI"]);
  		...
  		$listMenus = '<ul id="lof-easy-menu-'. $parent .'" class="lof-easy-menu '. $class .'"'.(!$parent ? ' style="display:none;"' : '').'>';
  		$pl_menulinks = $this->getsListMenus($cookie->id_lang, $parent);
		...
		foreach ($pl_menulinks as $pl_menulink) {
			...
			// Add submenu
			if($pl_menulink['subitems'])
				$listMenus .= $this->displayLofListMenu($pl_menulink['id_pl_menu']);
	...
	public function hookTop($param)
	{
		...
		$smarty->assign('MENU_RESPONSIVE', $this->displayLofListMenu(0,'lof'));

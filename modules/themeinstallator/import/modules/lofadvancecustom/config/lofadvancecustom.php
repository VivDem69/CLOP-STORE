...
<link href="<?php echo __PS_BASE_URI__;?>css/jquery.fancybox-1.3.4.css"  type="text/css" rel="stylesheet"/>
...
?>
	...
	<?php if(count($blocks) < LOF_MODULE_ADVANCE_CUSTOM_LIMIT_BLOCK){ ?>
		<a onclick="return false;" class="lofaddnew-block-item lofaddnew-block-<?php echo $id_position;?>" href="<?php echo _MODULE_DIR_.$this->name.'/popup.php?id_position='.$id_position.'&addBlock&bo_theme='.$employee->bo_theme.'&id_divposition=lofposition-'.$id_position.'&id_lang='.$cookie->id_lang.'&nb='.count($blocks).'&token='.Tools::getValue('token').'&secure_key='.$this->secure_key;?>" title="add"><img border="0" src="../img/admin/add.gif"/><?php echo $this->l('Add new block');?></a><br/>
	<?php
	}
	if($blocks){
		foreach($blocks as $key=>$b){
				...
				<form id="form-<?php echo $b['id_loffc_block'];?>" action="<?php echo $this->base_config_url.'&rand='.rand();?>" method="post" enctype="multipart/form-data">
					...
					foreach($this->_languages as $language)
						echo '<div id="title'.$b['id_loffc_block'].'_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
							<input size="30" type="text" name="title_'.$language['id_lang'].'" value="'.htmlentities($obj->{'title'}[(int)($language['id_lang'])], ENT_COMPAT, 'UTF-8').'" /><sup> *</sup>
						</div>';
